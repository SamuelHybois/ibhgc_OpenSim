function st_efforts=harmonised_ecriture_mot_effortV2(data_c3d, nom_fichier,chemin_c3d,M_pass_Vicon_OS, repertoire_sortie, temps_crop_inf, temps_crop_sup,st_protocole)
...
for i_plateformes=1:length(numeros_plateformes)
    ...
    %     coins=data_c3d.fp_data.FP_data(4).corners;
    %     centre=(coins(:,1)+coins(:,2)+coins(:,3)+coins(:,4))*0.25;
    %     longueur=max(size(cur_F,1),size(cur_F,2));
    %     cur_P=ones(1,longueur)'*centre'*(1/1000);
    cur_P=0*cur_F;
