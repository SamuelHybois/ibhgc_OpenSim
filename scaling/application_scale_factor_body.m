function tree_out=application_scale_factor_body(tree,scaling_factors)
tree_out=tree;
...
for i_body=1:nb_bodies
    ...
    if ~strcmp(cur_body,'ground') && isfield(scaling_factors,cur_body)
        tree_out.Model.BodySet.objects.Body(i_body).VisibleObject.scale_factors=scaling_factors.(cur_body);
    end
end
