function DOMnode = xml_writeOSIM_TJ(filename, tree, RootName, Pref)
...
% 
% Thomas Joubert MAJ
% J’ai modifié les deux fonctions (lecture et écriture) pour stocker et 
% restituer l’ordre de tous les tags XML (et accessoirement préserver la précision des valeurs numériques qui étaient tronquées).
% Si on veut juste modifier des valeurs/coordonnées dans le fichier 
% (sans ajouter de nouveaux tags ni en enlever) il suffit de faire 
% (en n’oubliant pas la partie en gras…) :
% 
% A=xml_readOSIM('MoBL_ARMS_module2_4_allmuscles.osim',struct('Sequence',true));
% 
% % ... modifications ...
% 
% xml_writeOSIM('MoBL_ARMS_module2_4_allmuscles.out',A,'OpenSimDocument');  
% 
% Si on veut ajouter (ou enlever) des tags, c’est plus compliqué, il faut aussi les rajouter (ou les enlever) dans le champ SEQUENCE de l’objet parent (à la place où on veut qu’ils apparaissent) sinon ils ne seront pas écrits dans le fichier exporté.
...
%  Pref - preferenced
  ...
  elseif (isstruct(s))  % if struct than deal with each field separatly
    if isfield(s,'SEQUENCE')
      node = xml.createElement(name);
      for i=1:size(s.SEQUENCE,1)
        kind=s.SEQUENCE{i,1};
        index=s.SEQUENCE{i,2};

        if isempty(s.(kind))
          struct2DOMnode(xml,node,[],kind,Pref);
        elseif iscell(s.(kind)) || isstruct(s.(kind))
          struct2DOMnode(xml,node,s.(kind)(index),kind,Pref);
        elseif index==1
          struct2DOMnode(xml,node,s.(kind),kind,Pref);
        end
      end

      if isfield(s,'ATTRIBUTE')
        attName = fieldnames(s.ATTRIBUTE);       % get names of all the attributes
        for k=1:length(attName)        % attach them to the node 
          att = xml.createAttribute(varName2str(attName(k)));
          att.setValue(var2str(s.ATTRIBUTE.(attName{k})));
