function [tree, RootName, DOMnode] = xml_readOSIM_TJ(xmlfile, Pref)
...
% 
% Si on veut ajouter (ou enlever) des tags, c’est plus compliqué, il faut aussi les rajouter (ou 
...
if (nargin>1)
  ...
  if (isfield(Pref, 'Sequence' )), DPref.Sequence  = Pref.Sequence;  end
...
if (isstruct(s))
  ...
  if Pref.Sequence
    s.SEQUENCE = [];
    for i=1:nField
      for j=1:numel(o.(fields{i}))
        s.SEQUENCE=[s.SEQUENCE;{fields{i} j o.(fields{i})(j)}];
      end
    end
    s.SEQUENCE=sortrows(s.SEQUENCE,3);
    s.SEQUENCE=s.SEQUENCE(:,1:2);
  end
end
...
if (isstruct(s))  
  ...
  for iItem=1:length(s)       % for each struct in the array - usually one
    for iField=1:length(fields)
      ...
      if Pref.Sequence && strcmp(field,'SEQUENCE')
          continue;
      end
      x = s(iItem).(field);   
