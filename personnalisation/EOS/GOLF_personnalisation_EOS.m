...
% 
function [model_new_9]=GOLF_personnalisation_EOS(model_old,repertoire_wrl_EOS,rep_billes_EOS, Protocol, sexe, repertoire_ddr)
...
% model_old=xml_readOSIM('E:\ISBS_data_ID\ReBe_M_1968\golf_003_RB\traitement_OpenSim\memb_inf.osim');
...
file_ddr_tibia_r=[repertoire_ddr 'tibia_peroneD.ddr'];
file_ddr_tibia_l=[repertoire_ddr 'tibia_peroneG.ddr'];
...
%attribution du centre du bassin dans la structure
% % centre_bassin=MH_R0Rlocal_bassin(1:3,4)';
% % h=['model_new.Model.BodySet.object.Body(' nb_bassin ').Joint.CustomJoint.location_in_parent=[' num2str(centre_bassin) '];'];eval(h)

%récupération de la séquence d'axe
% h=['axe1=model_new.Model.BodySet.objects.Body(' nb_bassin ').Joint.CustomJoint.SpatialTransform.TransformAxis(1).axis;'];
% eval(h)
% h=['axe2=model_new.Model.BodySet.objects.Body(' nb_bassin ').Joint.CustomJoint.SpatialTransform.TransformAxis(2).axis;'];
% eval(h)
% h=['axe3=model_new.Model.BodySet.objects.Body(' nb_bassin ').Joint.CustomJoint.SpatialTransform.TransformAxis(3).axis;'];
% eval(h)
% seq=trouver_seq(axe1,axe2,axe3);

% %évaluation des angles selon la séquence d'axes
% h=['[a_bassin b_bassin c_bassin]=axe_mobile_' seq '(MH_R0Rlocal_bassin(1:3,1:3));'];
% eval(h)

% %attribution de la séquence d'axes dans la structure
% angles=[a_bassin b_bassin c_bassin];
% angles=angles*2*pi/360;
% h=['model_new.Model.BodySet.objects.Body(' nb_bassin ').Joint.CustomJoint.orientation_in_parent=[' num2str(angles) '];'];
% eval(h)
...
% model_new_9.Model.BodySet.objects.Body(9).Joint.CustomJoint.location=decallage';
% % % % % % % % % % % % % %
% model_new_9.Model.BodySet.objects.Body(9).Joint.CustomJoint.location=centre_condile_EOS_Rtib_l+[0
...
% model_new_9.Model.BodySet.objects.Body(4).Joint.CustomJoint.location=decallage';
...
% model_new_9.Model.BodySet.objects.Body(9).Joint.CustomJoint.orientation_in_parent=[0 0 0];
RootName ='OpenSimDocument';
% faire une disctinction en fonction de la version de Matlab
% version 2014
% xml_writeOSIM('modele_perso_ReBe.osim',model_new_9,RootName,struct('StructItem',false,'CellItem',false));
% version 2011
cd(repertoire_sortie)
xml_writeOSIM('modele_EOS_ISBS.osim',model_new_9,RootName);
